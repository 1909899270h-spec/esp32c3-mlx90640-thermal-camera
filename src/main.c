...
static const char* html_start = "<!doctype html><html lang=\"zh-CN\"><head><meta charset=\"UTF-8\">"
    "<meta name=\"viewport\" content=\"width=device-width,initial-scale=1\">"
    "<title>MLX90640 热成像相机</title><style>";

static const char* html_style = "html,body{margin:0;width:100%;height:100%;overflow:hidden;background:#050505;color:#f5f5f5;font-family:Arial,'Microsoft YaHei',sans-serif}"
    ".app{height:100vh;display:grid;grid-template-rows:auto 1fr auto;background:#050505}"
    ".toolbar{display:flex;align-items:center;gap:10px;padding:10px 14px;background:#101010;border-bottom:1px solid #262626;flex-wrap:wrap}"
    ".title{font-size:17px;font-weight:700;margin-right:10px;white-space:nowrap}"
    "button,select{height:34px;border:1px solid #2e6f3f;background:#17823a;color:#fff;border-radius:4px;padding:0 12px;font-size:14px;cursor:pointer}"
    "button.secondary{background:#252525;border-color:#4a4a4a}button:disabled{opacity:.45;cursor:default}"
    ".metric{color:#d7d7d7;font-size:14px;white-space:nowrap}"
    ".stage{min-height:0;display:flex;align-items:center;justify-content:center;gap:14px;padding:10px;box-sizing:border-box;background:#000}"
    "#thermalCanvas{width:min(calc(100vw - 96px),calc((100vh - 112px)*1.3333));height:min(calc(100vh - 112px),75vw);background:#000;image-rendering:auto;display:block}"
    ".color-scale{height:min(calc(100vh - 132px),75vw);min-height:260px;display:grid;grid-template-rows:auto 1fr auto auto;align-items:center;gap:8px;color:#f2f2f2;font-size:13px;text-align:center}"
    ".color-bar{width:26px;height:100%;border:1px solid #444;background:linear-gradient(to top,rgb(8,18,82),rgb(28,68,180),rgb(35,132,220),rgb(48,178,96),rgb(232,210,44),rgb(224,80,44),rgb(245,238,215))}"
    ".footer{display:flex;justify-content:center;gap:32px;padding:9px 12px;background:#101010;border-top:1px solid #262626;color:#dcdcdc;font-size:14px}"
    "@media(max-width:720px){.toolbar{gap:7px;padding:8px}.title{width:100%}button,select{height:32px;padding:0 9px}.footer{gap:14px;font-size:13px;flex-wrap:wrap}.stage{gap:8px;padding:8px}#thermalCanvas{width:calc(100vw - 70px)}.color-scale{min-height:220px;font-size:12px}.color-bar{width:22px}}";

static const char* html_head_end = "</style></head><body>";

static const char* html_body = "<main class=\"app\">"
...
static const char* html_script = "<script src=\"/web_script.js?v=20260615\"></script></body></html>";
...
static int mlx90640_init_status = -999;
static int mlx90640_eeprom_status = -999;
static int mlx90640_extract_status = -999;
static int mlx90640_refresh_status = -999;
static int mlx90640_chess_status = -999;
static int last_frame_status = 0;
static uint32_t frame_ok_count = 0;
static uint32_t frame_fail_count = 0;
static esp_err_t last_i2c_error = ESP_OK;
static uint32_t i2c_error_count = 0;
static uint8_t last_i2c_error_addr = 0;
static uint16_t last_i2c_error_reg = 0;
...
int MLX90640_I2CRead(uint8_t slaveAddr, unsigned int startAddress, unsigned int nWordsRead, uint16_t *data) {
    ...
    if (ret != ESP_OK) {
        last_i2c_error = ret;
        i2c_error_count++;
        last_i2c_error_addr = slaveAddr;
        last_i2c_error_reg = (uint16_t)startAddress;
...
int MLX90640_I2CWrite(uint8_t slaveAddr, unsigned int writeAddress, uint16_t data_val) {
    ...
    if (ret != ESP_OK) {
        ...
        i2c_error_count++;
        last_i2c_error_addr = slaveAddr;
        last_i2c_error_reg = (uint16_t)writeAddress;
...
static esp_err_t mlx90640_sensor_init(void) {
    ...
    mlx90640_refresh_status = status;
    if (status != 0) {
...
static esp_err_t status_get_handler(httpd_req_t *req)
{
    ...
    esp_err_t probe_ret = i2c_probe_address(MLX90640_I2C_ADDR);
    int written = snprintf(json_buffer, sizeof(json_buffer),
        "{"
        "\"sda_gpio\":%d,"
        "\"scl_gpio\":%d,"
        "\"expected_addr\":\"0x%02X\","
        "\"mlx90640_probe_ok\":%s,"
        "\"mlx90640_initialized\":%s,"
        "\"init_status\":%d,"
        "\"eeprom_status\":%d,"
        "\"extract_status\":%d,"
        "\"refresh_status\":%d,"
        ...
        "\"last_i2c_error\":\"%s\","
        "\"i2c_error_count\":%lu,"
        "\"last_i2c_error_addr\":\"0x%02X\","
        "\"last_i2c_error_reg\":\"0x%04X\""
        "}",
        (int)I2C_MASTER_SDA_IO,
        (int)I2C_MASTER_SCL_IO,
        MLX90640_I2C_ADDR,
        probe_ret == ESP_OK ? "true" : "false",
        mlx90640_initialized ? "true" : "false",
        mlx90640_init_status,
        mlx90640_eeprom_status,
        mlx90640_extract_status,
        mlx90640_refresh_status,
        mlx90640_chess_status,
        taskRunning ? "true" : "false",
        last_frame_status,
        (unsigned long)frame_ok_count,
        (unsigned long)frame_fail_count,
        esp_err_to_name(last_i2c_error),
        (unsigned long)i2c_error_count,
        last_i2c_error_addr,
        last_i2c_error_reg);

    if (written < 0 || written >= sizeof(json_buffer)) {
        httpd_resp_send_500(req);
        return ESP_FAIL;
    }
    ...
    return httpd_resp_send(req, json_buffer, written);
}

static esp_err_t i2c_scan_get_handler(httpd_req_t *req)
{
    char json_buffer[1536];
    int pos = 0;
    int found_count = 0;
    bool first = true;
    bool mlx_found = false;

    pos += snprintf(json_buffer + pos, sizeof(json_buffer) - pos,
        "{\"sda_gpio\":%d,\"scl_gpio\":%d,\"expected_addr\":\"0x%02X\",\"found\":[",
        (int)I2C_MASTER_SDA_IO,
        (int)I2C_MASTER_SCL_IO,
        MLX90640_I2C_ADDR);
    ...
    for (uint8_t addr = 0x03; addr <= 0x77; addr++) {
        if (i2c_probe_address(addr) == ESP_OK) {
            int written = snprintf(json_buffer + pos, sizeof(json_buffer) - pos,
                "%s\"0x%02X\"", first ? "" : ",", addr);
            if (written < 0 || written >= sizeof(json_buffer) - pos) {
                httpd_resp_send_500(req);
                return ESP_FAIL;
            }
            pos += written;
    ...
    int written = snprintf(json_buffer + pos, sizeof(json_buffer) - pos,
        "],\"count\":%d,\"mlx90640_seen\":%s,"
        "\"hint\":\"MLX90640 should appear at 0x33. If found is empty, check 3.3V, GND, SDA, SCL, pullups and loose contacts.\"}",
        found_count,
        mlx_found ? "true" : "false");
    ...
    httpd_resp_set_type(req, "application/json");
    httpd_resp_set_hdr(req, "Cache-Control", "no-store");
    return httpd_resp_send(req, json_buffer, pos + written);
}
...
static const httpd_uri_t i2c_scan_uri = {
    .uri       = "/i2c-scan",
    .method    = HTTP_GET,
    .handler   = i2c_scan_get_handler,
    .user_ctx  = NULL
};
